            ...
            for (size_t i = 1; i <= s_size; ++i) {
                for (size_t j = 1; j <= j_size; ++j) {
                    if (scroll_name[i - 1] == journal[j - 1]) {
                        //cout << "scroll_name[" << i << " - 1] == journal[" << j << " - 1]: " << scroll_name[i - 1] << endl;
                        lcs_table[i][j] = lcs_table[i - 1][j - 1] + 1;
                    } 
                    else {
                        lcs_table[i][j] = max(lcs_table[i][j - 1], lcs_table[i - 1][j]);
                        //cout << "lcs_table[" << i << "][" << j << "] = max(lcs_table[" << i << "][" << j << " - 1], lcs_table[" << i << " - 1][" <<j << "])";
                        //cout << " = max(" << lcs_table[i][j - 1] << ", " << lcs_table[i - 1][j] << ")";
                        //cout << " = " << lcs_table[i][j] << endl;
                    } 
                }
            }
